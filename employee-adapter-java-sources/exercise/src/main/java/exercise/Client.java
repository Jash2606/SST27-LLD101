...
public class Client {
  public static void main(String[] args) {
    ...
    // TODO: Wrap each legacy object with the right adapter and collect into one
    // list
    List<EmployeeAdaptor> all = new ArrayList<>();
     for(EmployeeCSV e: csvRows){
        all.add(new AdaptEmployeeCSV(e));
     }
     for(EmployeeDB e: dbRows){
         all.add(new AdaptEmployeeDB(e));
     }
     for(EmployeeLDAP e: ldapRows){
         all.add(new AdaptEmployeeLDAP(e));
     }
